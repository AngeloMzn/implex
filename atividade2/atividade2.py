# Aluno: Ângelo Vinicius Hernandez Mazarin 
# RGA: 202219060761
import time
def valorMax(n, tabelaPrecos):
    ...
    for i in range(1, n + 1):
        ...
        valorMax[i] = vendaMax
    return valorMax[n]
def corteGuloso(n, tabelaPrecos):
    valor_total = 0
    while n > 0:
        melhor_densidade = 0
    ...
    return valor_total
def imprimir_tabela(tamanhos, vDps, tDp, vGds, tGd):
...
for a in range(20):  
    ...
    vDps.append(vDp)
    inicio_gd = time.time()
    vGd = corteGuloso(n, tabelaPrecosGd)
    fim_gd = time.time()
...
tabela = imprimir_tabela(tamanhos, vDps, tDp, vGds, tGd)
